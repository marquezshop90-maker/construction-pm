...
import { HardHat, LayoutDashboard, FolderOpen, Calculator, Upload, Plus } from 'lucide-react'
      ...
      <div className="flex items-center h-14 px-6">
        <Link href="/dashboard" className="flex items-center gap-3 mr-8 flex-shrink-0">
          ...
          <div className="leading-tight">
            <span className="block text-white text-xs font-black tracking-widest uppercase">Construction</span>
            <span className="block text-orange-400 text-[10px] font-semibold tracking-wider uppercase">Project Manager</span>
          </div>
        </Link>
            ...
            return (
              <Link key={href} href={href}
                className={`flex items-center gap-2 px-3 py-2 rounded text-sm font-medium transition-all ${
                  active ? 'bg-orange-500 text-white' : 'text-slate-400 hover:text-white hover:bg-slate-700/60'
                }`}>
